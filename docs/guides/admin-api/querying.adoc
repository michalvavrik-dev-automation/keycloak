...
The `null` literal is accepted only with the `eq` and `ne` operators.
Comparisons with `null` follow the three-valued logic of SQL, so `description eq null` and `description ne null` never match a client, not even a client without a description.
For the same reason, `description ne "internal"` does not match clients without a description.
To test for presence, use the presence operator `pr`, which is true when a field has a value, and negate it to test for absence.
For example, `description pr` matches clients with a description, and `not description pr` matches clients without one.
For details, see link:{adminguide_link}#_scim_filtering_null_values[Comparing with null values] in the {adminguide_name}.
...
To find clients where such a field is absent, negate the presence operator `pr`, which is true only when the field has a value.
For example, `not auth.method pr` matches every client without an authentication method.
Comparing the field with `null` never matches, as described in <<Null values>>.
