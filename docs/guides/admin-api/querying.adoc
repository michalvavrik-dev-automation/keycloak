...
A comparison with a field that has no value is never true.
For example, `auth.method ne "client-secret"` does not return SAML clients or public clients, because they have no authentication method. Use `not auth.method pr` to find them.
`description eq null` never matches either, and `null` is accepted only with `eq` and `ne`.
