...
Use `not <field> pr` to match clients where the field is absent.
...
For multi-valued fields (type `Set<String>` in the tables above), the `eq`, `ne`, `co`, `sw`, and `ew` operators match if *any* element in the collection satisfies the condition:
...
* `roles ne "admin"` - matches clients that have at least one role other than `admin`
...
To exclude clients that have a value, negate the condition with `not`.
For example, `not (roles eq "admin")` matches clients that do not have a role named `admin`, including clients without any roles.
