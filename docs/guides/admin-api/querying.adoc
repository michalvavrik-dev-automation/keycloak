...
Comparisons with `null` follow the three-valued logic of SQL, so `field eq null` and `field ne null` never match a client, not even a client where the field is absent.
Use `field pr` to find clients where a field has a value, and `not field pr` to find clients where it has none.
For the same reason, `field ne "value"` does not match clients where the field is absent.
...
Use `not <field> pr` to match clients where the field is absent. `<field> eq null` never matches, as described in <<Null values>>.
