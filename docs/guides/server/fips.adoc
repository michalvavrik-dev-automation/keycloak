...
[source]
----
FIPS1402Provider created: KC(BCFIPS version 2.0103 Approved Mode, FIPS-JVM: enabled)
----
...
Enforcing post-quantum cryptography for incoming HTTPS connections by setting the `pqc-http-in` or `pqc-http-management` option to `enforce-hybrid` is not supported in FIPS mode,
as the BouncyCastle JSSE provider does not support a hybrid post-quantum key exchange. For details, see <@links.server id="enabletls" />.
...
BouncyCastle FIPS requires TLS certificates with a Subject Alternative Name (SAN), with no fallback to using
the Common Name (CN) from the certificate. Due to an issue in older versions of BouncyCastle FIPS the fallback was
enabled by default, which is no longer the case.

If you see an error such as `No hostname specified for HTTPS endpoint ID check` the recommended solution is to
update the certificates to include a Subject Alternative Name (SAN).

As an alternative temporary solution enable the Common Name (CN) fallback with:

[source, bash]
----
export JAVA_OPTS_APPEND="-Dorg.bouncycastle.jsse.hostname_check_cn_fallback=true"
----
