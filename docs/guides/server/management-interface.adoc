...
includedOptions="http-management-* https-management-* pqc-http-management legacy-observability-interface">
...
The post-quantum cryptography mode is inherited from the `pqc-http-in` option in the same way, and can be set for the management interface specifically with the `pqc-http-management` option.
