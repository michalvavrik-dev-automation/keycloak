...
includedOptions="truststore-paths truststore-paths-reload-period truststore-kubernetes-enabled tls-hostname-verifier">
...
The existing Java default truststore certs will always be trusted.  If you need additional certificates, which will be the case if you have self-signed or internal certificate authorities that are not recognized by the JRE, they can be included in the `conf/truststores` directory or subdirectories.  The certs may be in PEM files, or passwordless PKCS12 files with extension `.p12`, `.pfx`, or `.pkcs12`.  A PKCS12 file must have no MAC or an empty password; password-protected PKCS12 files are not supported.
...
The generated system truststore type is automatically adjusted to the current FIPS mode and the active crypto provider. By default, and in FIPS non-strict mode, the generated truststore remains PKCS12 when PKCS12 is supported. In FIPS strict mode, the generated truststore uses BCFKS. The `--https-trust-store-type` option applies to an explicit `--https-trust-store-file`; it does not override the generated system truststore type.
...
== Reloading the system truststore

By default, the system truststore is built once at startup and stays fixed for the lifetime of the server. To pick up rotated CA certificates without a restart - for example, when a certificate authority is renewed - set the `--truststore-paths-reload-period` option to a duration. Reloading is opt-in: when the option is unset, the system truststore is never reloaded automatically.

When set, {project_name} periodically re-reads the files and directories referenced by `--truststore-paths`, together with the automatically discovered default CA certificates (such as the Kubernetes and OpenShift cluster CAs), and rebuilds the system truststore. The period controls how quickly a certificate rotation is detected.

For example, to reload the system truststore every hour:

<@kc.start parameters="--truststore-paths=/opt/truststore/myTrustStore.pem --truststore-paths-reload-period=1h" />

Reloading is graceful and is intended to run proactively, before the current certificates expire. Requests and connections already in progress continue to use the previous truststore and complete normally; only new requests and connections use the reloaded truststore. Reloading works in both FIPS strict and non-strict modes.

NOTE: Pooled LDAP and LDAPS connections continue to use the previous truststore until they are recycled. New LDAP connections use the reloaded truststore.

== Cluster CA certificates (Kubernetes and OpenShift)

When running {project_name} on Kubernetes or OpenShift, the server will automatically include well-known cluster certificate authorities in the system truststore if they are present on the filesystem:
...
<@kc.start parameters="--truststore-kubernetes-enabled=false" />
