...
By default, the system truststore is built once at startup and stays fixed for the lifetime of the server. If your trusted certificates are rotated while {project_name} is running - for example, when a certificate authority is renewed - you would normally need to restart the server to pick up the changes.

To avoid a restart, use the `--truststore-paths-reload-period` option to have {project_name} reload the system truststore periodically. This behavior is opt-in and is disabled unless the option is set. The value is a duration.

When enabled, {project_name} re-reads the PEM and PKCS12 files referenced by the `--truststore-paths` option, together with the automatically discovered CA certificates (such as the Kubernetes and OpenShift cluster CAs), rebuilds the system truststore, and applies the updated material to outgoing TLS connections - including the HTTP client, LDAP and LDAPS connections, email (SMTP), and the reverse-proxy client certificate lookup - without requiring a restart.

On each reload, the referenced files are re-read and any referenced directories are re-scanned, so certificate changes - including files added to or removed from a scanned directory - are picked up. The list of `--truststore-paths` entries is fixed at startup; changing that list requires a restart. The truststore is reloaded on every interval, so set the period to the longest interval that still meets your certificate-rotation needs.
